    ...
    >
      ...
      >
        ...
        >
          ...
          <img
            src={item.isLocal ? item.imageUrl : `http://localhost:5000${item.imageUrl}`}
            alt={item.title}
        ...
        <div className="p-5">
          <p className="line-clamp-1 text-sm font-bold tracking-tight text-slate-900 transition-colors group-hover:text-brand-500 dark:text-slate-100">{item.title}</p>
          <p className="mt-1 text-xs font-semibold uppercase tracking-wider text-slate-400 dark:text-slate-500">{item.category}</p>
        </div>
      </a>
    </article>
  )
